...
namespace FinanceTracker.Client.Services
{
    public class DataService : IDataService
    {
        ...
        public async Task<SummaryData> LoadTotalEarnings()
        {
            var totalEarnings = await _httpClient.GetFromJsonAsync<Earning[]>("api/Earnings");
            sumEarning = totalEarnings.Sum(t => t.Amount);
            var summaryEarning = new SummaryData { Amount = sumEarning, Label = "Income" };

            return summaryEarning;
        }

        public async Task<SummaryData> LoadTotalExpenses()
        {
            var totalExpenses = await _httpClient.GetFromJsonAsync<Expense[]>("api/Expenses");
            sumExpense = totalExpenses.Sum(x => x.Amount);
            var summaryExpense = new SummaryData { Amount = sumExpense, Label = "Expenses" };

            return summaryExpense;
        }

        public async Task<SummaryData> CalculateBalance()
        {
            var sumBalance = sumEarning - sumExpense;
            var summaryBalance = new SummaryData { Amount = sumBalance, Label = "Balance" };
